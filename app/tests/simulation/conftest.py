...
from app.tests.simulation.simulator import Simulator
...
from app.utils.misc import find_perms, gen_random_str
...
def mock_env(monkeymodule: Any) -> None:
    monkeymodule.setattr("app.config.config.settings.environment", "DEVELOPMENT")
...
def mock_account_create_mail(
    ...
    monkeymodule.setattr(
        "app.user.cruds.send_account_create_mail",
        mock_send_account_create_mail,
    )
...
def mock_password_reset_mail(
    ...
    monkeymodule.setattr(
        "app.user.cruds.send_password_reset_link_mail",
        mock_send_password_reset_mail,
    )
...
def mock_password_change_mail(
    ...
    monkeymodule.setattr(
        "app.user.cruds.send_change_password_mail",
        mock_send_password_change_mail,
    )


@pytest.fixture(scope="session")
def create_account_mailbox(
    mock_account_create_mail: Any, tmp_path_factory: TempPathFactory
) -> Path:
    tmp_dir = tmp_path_factory.getbasetemp()
...
def password_reset_mailbox(
    mock_password_reset_mail: Any, tmp_path_factory: TempPathFactory
) -> Path:
    tmp_dir = tmp_path_factory.getbasetemp()
...
def password_change_mailbox(
    mock_password_change_mail: Any, tmp_path_factory: TempPathFactory
) -> Path:
    tmp_dir = tmp_path_factory.getbasetemp()
...
def super_user(
    ...
    user = user_cruds.create_user(crud_util, user_data, is_admin=True)
...
@pytest.fixture(scope="function")
def creator(crud_util: CrudUtil) -> Any:
    user_data: user_schemas.UserIn = gen_user()

    # check if station_manager_group exists, if not create it
    try:
        station_manager_group = crud_util.get_model_or_404(
            ac_models.Group, {"name": "station_manager_group"}
        )
    except Exception:
        station_manager_group = ac_models.Group(name="station_manager_group")

        crud_util.db.add_all(
            [
                station_manager_group,
            ]
        )
        crud_util.db.commit()
    ...
    return user_schemas.UserSchema.model_validate(user)
...
def inactive_admin_user(crud_util: CrudUtil, mock_account_create_mail: Any) -> Any:
    ...
    user = user_cruds.update_user(
        crud_util, str(user.uuid), user_schemas.UserUpdate(is_active=False)
    )
    return user
...
def admin_users(
    crud_util: CrudUtil, mock_account_create_mail: Any
...
def su_token_headers(
    client: TestClient, super_user: user_models.User, create_account_mailbox: Path
) -> dict[str, str]:
    password = create_account_mailbox.read_text().strip()

    response = client.post(
        "/auth/token", json={"email": super_user.email, "password": password}
    )
...
def invalid_token_headers() -> dict[str, str]:
    return {"Authorization": f"Bearer {gen_random_str()}"}
