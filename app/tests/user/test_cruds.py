...
def test_create_admin_user(
    ...
    user_data: schemas.UserIn = gen_user()
    user = cruds.create_user(crud_util, user_data, is_admin=True)

    admin_password = create_account_mailbox.read_text().strip()

    assert verify_password_hash(admin_password, str(user.password_hash))
    assert str(user.email) == user_data.email
...
def test_get_user_by_email(crud_util: CrudUtil, user: models.User) -> Any:
    user = cruds.get_user_by_email(crud_util, str(user.email))
...
def test_get_user_by_email_not_found(crud_util: CrudUtil) -> Any:
    with pytest.raises(HTTPException):
        cruds.get_user_by_email(
            crud_util,
            str("somenonexistentmail@mail.com"),
        )


def test_get_user_by_uuid(crud_util: CrudUtil, user: models.User | Any) -> Any:
    user = cruds.get_user_by_uuid(crud_util, str(user.uuid))
    assert str(user.uuid) == user.uuid
    assert hasattr(user, "id")
...
def test_update_user(crud_util: CrudUtil, user: models.User) -> Any:
    ...
    assert str(user.email) != user_data.email
    assert str(user.firstname) != user_data.firstname

    updated_user: models.User = cruds.update_user(crud_util, str(user.uuid), user_data)
...
def test_update_user_not_found(crud_util: CrudUtil) -> Any:
    with pytest.raises(HTTPException):
        user_data: schemas.UserUpdate = gen_user_update()
        cruds.update_user(crud_util, gen_uuid(), user_data)
...
def test_delete_user(crud_util: CrudUtil, user: models.User) -> Any:
    ...
    with pytest.raises(HTTPException):
        cruds.get_user_by_uuid(crud_util, str(user.uuid))
...
def test_get_user_by_phone(crud_util: CrudUtil, user: models.User) -> Any:
    user = cruds.get_user_by_phone(crud_util, str(user.phone))
    assert str(user.phone) == user.phone
    assert hasattr(user, "id")


def test_get_user_by_phone_not_found(crud_util: CrudUtil) -> Any:
    with pytest.raises(HTTPException):
        cruds.get_user_by_phone(
            crud_util,
            gen_random_phone(),
        )
...
def test_authenticate_user(
    crud_util: CrudUtil, admin_user: models.User, create_account_mailbox: Path
) -> Any:
    admin_password = create_account_mailbox.read_text().strip()
    authenticated_user = cruds.authenticate_user(
        crud_util, str(admin_user.email), admin_password
    )
...
def test_authenticate_inactive_user(
    ...
    cruds.update_user(crud_util, str(user.uuid), schemas.UserUpdate(is_active=False))
...
def test_request_password_reset(
    crud_util: CrudUtil, user: models.User, password_reset_mailbox: Path
) -> Any:
    response = cruds.reset_password(crud_util, str(user.email))

    password = password_reset_mailbox.read_text().strip()
    assert len(password) > 0
    assert (
        response["detail"]
        == "Your password has been sent to your email or phone if it exists on the system. "
    )
...
def test_request_password_reset_invalid_email(
    ...
    response = cruds.reset_password(crud_util, str(gen_email()))
...
def test_reset_password(
    ...
    cruds.reset_password(crud_util, str(user.email))
    ...
    db_user = cruds.get_user_by_uuid(crud_util, str(user.uuid))

    assert "Password reset successful" in response["detail"]
    assert new_password in response["detail"]
    assert verify_password_hash(new_password, str(db_user.password_hash))  # type:ignore
...
def test_list_admin_users_filter(
    ...
    db_users: schemas.UserList = cruds.list_admin_users(
        crud_util,
        schemas.AdminUserFilter(email=str(admin_users[0].email)),
        limit=1,
    )

    assert len(db_users.items) == 1
    assert db_users.items[0].email == admin_users[0].email
...
def test_change_admin_user_password(
    ...
    db_user = cruds.get_user_by_uuid(crud_util, str(admin_user.uuid))

    admin_password = password_change_mailbox.read_text().strip()

    assert verify_password_hash(
        admin_password, str(db_user.temp_password_hash)  # type:ignore
    )
